...
tokenizer = BertTokenizer.from_pretrained('/home/chg/program/project/python/nlp/EE/model/bert-event-extraction_mod_2/bert-base-uncased/vocab.txt',
...
class ACE2005Dataset(data.Dataset):
    def __init__(self, fpath):
        ...
        with open(fpath, 'r') as f:
            ...
            for item in data:
                ...
                self.triggers_li.append([PAD] + triggers + [PAD]) #===================================================== 原代码没有为触发词加上CLS、SEP对应的PAD，与word不对应，改加上
    ...
    def __getitem__(self, idx):
        ...
        tokens_x, entities_x, postags_x, is_heads,triggers_y = [], [], [], [],[]#======================================= 原代码没有为触发词加上token对应的PAD，与word不对应，改加上
        # 依次取句子中的每个词；w为词(CLS)，e为词对应的实体标签（[B-XXX]），p为词对应的词性标签(XXX)
        # for w, e, p in zip(words, entities, postags):
        for w, e, p,t in zip(words, entities, postags,triggers):  #===================================================== 原代码没有为触发词加上token对应的PAD，与word不对应，改加上
            # bert分词
            ...
            # 新增
            t = [t] + [PAD] * (len(tokens) - 1)                   #===================================================== 原代码没有为触发词加上token对应的PAD，与word不对应，改加上
            # tag转为索引
            p = [postag2idx[postag] for postag in p]
            # entity转为索引
            e = [[entity2idx[entity] for entity in entities] for entities in e]
            # 新增
            t = [trigger2idx[trigger] for trigger in t]           #===================================================== 原代码没有为触发词加上token对应的PAD，与word不对应，改加上
            ...
            tokens_x.extend(tokens_xx), postags_x.extend(p), entities_x.extend(e), is_heads.extend(is_head),triggers_y.extend(t)  #== 原代码没有为触发词加上token对应的PAD，与word不对应，改加上
        # 触发词类型（事件类型）转为索引，注意：triggers是长度是分词前的状态，为啥不加上分词的部分？
        # triggers_y = [trigger2idx[t] for t in triggers]           #=================================================== 原代码没有为触发词加上token对应的PAD，与word不对应，改加上
        # 分词后关键词的索引（非后缀，有实义的词）
        head_indexes = []
        ...
        return tokens_x, entities_x, postags_x, triggers_y, arguments, seqlen, head_indexes, words, triggers #========== 原代码没有为触发词加上token对应的PAD，与word不对应，改加上
